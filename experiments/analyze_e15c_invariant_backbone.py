"""Frozen E15-C v1.1 matched-capacity invariant-backbone bootstrap."""
...
INVARIANT_ENSEMBLE = "invariant_residual_ensemble"
FREE = "free_baseline"
EXPOSURES = ("low", "medium", "high")
BOOTSTRAP_REPLICATES = 5000
BOOTSTRAP_SEED = 151606
...
def run(rows_path: Path, out_path: Path) -> None:
    ...
    with rows_path.open(newline="") as handle:
        for row in csv.DictReader(handle):
            if row["policy"] in {INVARIANT_ENSEMBLE, FREE}:
                per_task[row["prefix_exposure"]][int(row["task_id"])][row["policy"]] = float(row["nrmse"])
    if set(per_task) != set(EXPOSURES):
        raise RuntimeError(f"unexpected invariant-backbone exposure set: {sorted(per_task)}")
    ...
    for exposure in EXPOSURES:
        ...
        if any(set(task_values[task_id]) != {INVARIANT_ENSEMBLE, FREE} for task_id in task_ids):
            raise RuntimeError(f"incomplete invariant-backbone pair: {exposure}")
        differences = np.asarray([
            task_values[task_id][INVARIANT_ENSEMBLE] - task_values[task_id][FREE]
        ...
        output.append({
            ...
            "mean_C_inv": float(differences.mean()),
            "ci_2_5": ci_low,
            "ci_97_5": ci_high,
