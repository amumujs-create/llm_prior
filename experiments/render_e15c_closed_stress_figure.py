"""Render E15-C closed-mechanism stress from its frozen table only."""
...
def load(path: Path) -> list[dict[str, float]]:
    ...
    with path.open(newline="") as handle:
        for row in csv.DictReader(handle):
            rows[row["prefix_exposure"]] = {"mean": float(row["mean_C_closed"]), "lo": float(row["ci_2_5"]), "hi": float(row["ci_97_5"])}
    if set(rows) != set(EXPOSURES):
        raise RuntimeError(f"unexpected closed-stress exposure set: {sorted(rows)}")
...
def render(values: list[dict[str, float]], out_path: Path) -> None:
    ...
    ax.errorbar(x, mean, yerr=np.vstack((lower, upper)), marker="o", color="#8B2E3B", linewidth=2, capsize=4)
    ax.axhline(0.0, color="black", linewidth=0.8)
    ax.set_xticks(x, ("Low", "Medium", "High"))
    ax.set_xlabel("Prefix exposure")
    ax.set_ylabel("Mean paired NRMSE contrast")
    ax.set_title("E15-C closed mechanism − invariant residual ensemble")
