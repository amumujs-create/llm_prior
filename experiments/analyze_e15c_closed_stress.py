"""Frozen E15-C v1.1 closed-mechanism stress cellwise bootstrap."""
...
CLOSED = "closed_mechanism"
ENSEMBLE = "invariant_residual_ensemble"
EXPOSURES = ("low", "medium", "high")
BOOTSTRAP_REPLICATES = 5000
BOOTSTRAP_SEED = 151605
...
def run(rows_path: Path, out_path: Path) -> None:
    ...
    with rows_path.open(newline="") as handle:
        for row in csv.DictReader(handle):
            if row["policy"] in {CLOSED, ENSEMBLE}:
                per_task[row["prefix_exposure"]][int(row["task_id"])][row["policy"]] = float(row["nrmse"])
    if set(per_task) != set(EXPOSURES):
        raise RuntimeError(f"unexpected closed-stress exposure set: {sorted(per_task)}")
    ...
    for exposure in EXPOSURES:
        ...
        if any(set(task_values[task_id]) != {CLOSED, ENSEMBLE} for task_id in task_ids):
            raise RuntimeError(f"incomplete closed-stress pair: {exposure}")
        differences = np.asarray([
            task_values[task_id][CLOSED] - task_values[task_id][ENSEMBLE]
        ...
        output.append({
            ...
            "mean_C_closed": float(differences.mean()),
            "ci_2_5": ci_low,
            "ci_97_5": ci_high,
